...
public class JoinQueue extends Fragment {
    ...
    private void joinQueue(String barangayName) {
        ...
        db.runTransaction((Transaction.Function<Void>) transaction -> {
            ...
            DocumentSnapshot driverSnapshot = transaction.get(driverRef);

            boolean inQueue = userSnapshot.getBoolean("inQueue");
            int userBalance = userSnapshot.getLong("balance").intValue();
            int driverBalance = driverSnapshot.getLong("balance").intValue();

            if (!inQueue && userBalance >= 20 && driverBalance >= 20) {
                // Update inQueue and balance for user
                transaction.update(userRef, "inQueue", true);
                transaction.update(driverRef, "inQueue", true);
                transaction.update(userRef, "balance", userBalance - 20);

                // Update balance for driver
                transaction.update(driverRef, "balance", driverBalance - 20);

                // Create a transaction record in user transactions
                Map<String, Object> transactionData = new HashMap<>();
                transactionData.put("amount", 20);
                ...
                // Create a transaction record in driver transactions (if needed)
                DocumentReference newDriverTransactionRef = driverRef.collection("transactions").document();
                transaction.set(newDriverTransactionRef, transactionData);

                // Add to queue
                Map<String, Object> queueData = new HashMap<>();
                queueData.put("name", userSnapshot.getString("name"));
                queueData.put("tricycleNumber", driverSnapshot.getString("tricycleNumber"));
